...
function rstr2any(c, g) {
    var a = g.length,
        b, d, e, f, h, k = Array(Math.ceil(c.length / 2));
    for (b = 0; b < k.length; b++) k[b] = c.charCodeAt(2 * b) << 8 | c.charCodeAt(2 * b + 1);
    var m = Math.ceil(8 * c.length / (Math.log(g.length) / Math.log(2))),
        l = Array(m);
    for (d = 0; d < m; d++) {
        h = [];
        for (b = f = 0; b < k.length; b++)
            if (f = (f << 16) + k[b], e = Math.floor(f / a), f -= e * a, 0 < h.length || 0 < e) h[h.length] = e;
        l[d] = f;
        k = h
    }
    a = "";
    for (b = l.length - 1; 0 <= b; b--) a += g.charAt(l[b]);
    return a
}
